...
module.exports = function feeder(podcasts) {
	...
	throw new Error(this.errMsg('ipns hash', 'opts.ipns'));
	...
	var mp3Url = url.resolve(self.opts.ipns, podcasts[i].resourceId.videoId+'.mp3');
	...
	feed.item({
		...
		{'itunes:image': {
		    _attr: {
			href: url.resolve(self.opts.ipns, podcasts[i].resourceId.videoId+'.jpg')
		    }
		}}
	    ]
	});
