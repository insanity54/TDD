...
module.exports = function feeder(podcasts) {
	...
	var description = entities.encode(
	    podcasts[i].description.replace(/(?:\r\n|\r|\n)/g, '<br />')
	);

	//str = str.replace(/(?:\r\n|\r|\n)/g, '<br />');
        //console.log(entities.encode('<>"\'&©®')); // &lt;&gt;&quot;&apos;&amp;©®
	var mp3Path = path.join(self.opts.dest, podcasts[i].resourceId.videoId+'.mp3');
	var mp3Url = url.resolve(self.opts.podcastIPNS, podcasts[i].resourceId.videoId+'.mp3');
	...
	feed.item({
		...
		{'itunes:image': {
		    _attr: {
			href: url.resolve(self.opts.podcastIPNS, podcasts[i].resourceId.videoId+'.jpg')
		    }
		}}
	    ]
	});
