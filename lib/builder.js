...
Builder.prototype.build = function build(cb) {
	...
	console.log('BUILDER copying files to destination directory');
	for (var i=0; i<files.length; i++) {

	    // merge metadata files into one manifest
	    ...
	    // copy mp3s to the dist dir
	    if (path.extname(files[i]) === '.mp3') {
		
		// copy to dist directory
		//console.log('copying %s to %s', files[i], path.join(self.opts.dest, path.basename(files[i])));
		fs.copySync(
		    path.join(self.opts.src, files[i]),
		    path.join(self.opts.dest, path.basename(files[i]))
		);
		
	    }

	    
	    // copy thumbnail images to the dist dir
	...
	// sort items in manifest in reverse chronological order
	var manifest = _.chain(manifest)
            .sortBy(function(p) {
		return moment(p.publishedAt).valueOf();
	    })
            .reverse()
	    .value();
